    ...
    public IImage createPhotoImage(URL url) {
        ...
        img.addItemListener(new ItemListenerAdapter() {
            ...
            public void itemCursorChanged(IItem item,
                ...
                if (item.getParentItem().getTreeRootSpatial() instanceof JMEFrame) {
                    JMEFrame frame = (JMEFrame) item
                            .getParentItem().getTreeRootSpatial();
                    for (PickedSpatial pickedSpatial : spatialsList) {
                        if (pickedSpatial.getSpatial().equals(
                                frame.getMaskGeometry())) {
                            offParent = false;
                            message = message
                                    + "it's parent frame SCANS.";
                        }
                    }
                }

                if (parentContainerName.equals(stitcher.SCAN_NAME)
                        && offParent) {
                    for (PickedSpatial pickedSpatial : spatialsList) {
                        if ((pickedSpatial.getSpatial().toString())
                                .equals("maskGeometry")
                            ...
                            try {
                                ...
                                if (geometry.getParent() instanceof HotSpotFrame) {
                                    HotSpotFrame targetFrame = (HotSpotFrame) geometry
                                            .getParent();

                                    if (targetFrame.getName()
                                            .contains("hotspotf-")
                                            && !highlightedFrames
                                        ...
                                        IBorder border = targetFrame
                                                .getBorder();
                                        border.setColor(new ColorRGBA(
                                                1f, 0f, 0f, 0.6f));
                                        // targetFrame.setBorder(getHighlightedFrame());
                                        highlightedFrames
                                                .add(targetFrame);
                                    } else if (highlightedFrames
                                            .contains(targetFrame)) {
                                        message = message
                                                + "a hotspot frameprobably already red";
                                        for (IHotSpotFrame hotSpotFrame : highlightedFrames) {
                                            if (!hotSpotFrame
                                                    .equals(targetFrame)) {
                                                IBorder border = hotSpotFrame
                                                        .getBorder();
                                                border.setColor(new ColorRGBA(
                                                        1f, 1f, 1f,
                                                        0.6f));
                                ...
                                logger.debug("GetAttachmentItems: itemCursorReleased: Exception: "
                                        + e);
                            }
                        }
                    }
                ...
                if (!firstFrameFound && offParent) {
                    firstFrameFound = false;
                    message = message
                            + "something else than a hotspotframe or the parent (i.e. the mist)";
                    ...
                    for (PickedSpatial pickedSpatial : spatialsList) {
                        if ((pickedSpatial.getSpatial().toString())
                            ...
                            try {
                                ...
                                if (geometry.getParent() instanceof HotSpotFrame) {
                                    IHotSpotFrame targetFrame = (IHotSpotFrame) geometry
                                            .getParent();
                                    if (highlightedFrames
                                            .contains(targetFrame)) {
                                        firstFrameFound = true;
                                        IBorder border = targetFrame
                                                .getBorder();
            ...
            private JMERoundedRectangleBorder getHighlightedFrame() {
                JMERoundedRectangleBorder jMERoundedRectangleBorder = new JMERoundedRectangleBorder(
                        "randomframeborder", UUID.randomUUID(), 3,
                        15);
                jMERoundedRectangleBorder.setColor(new ColorRGBA(
                        1f, 0f, 0f, 0.6f));
            ...
            private JMERoundedRectangleBorder getNormalFrame() {
                JMERoundedRectangleBorder jMERoundedRectangleBorder = new JMERoundedRectangleBorder(
                        "randomframeborder", UUID.randomUUID(),
                        stitcher.BORDER_THICKNESS, 15);
                jMERoundedRectangleBorder.setColor(new ColorRGBA(
                        1f, 1f, 1f, 0.6f));
                return jMERoundedRectangleBorder;
            }
            ...
            public void itemCursorReleased(IItem item,
                ...
                if (parentContainerName
                        .equals(stitcher.BACKGROUND_NAME)
                        && offParent) {
                        ...
                        if ((pickedSpatial.getSpatial().toString())
                            ...
                            try {
                                ...
                                if (geometry.getParent() instanceof HotSpotFrame) {
                                    ...
                                    if (!targetFrame.isLocked()
                                            && targetFrame
                                                    .getName()
                                                    .contains(
                                                            "back-")
                                            && (parentContainerName
                                                    .equals(stitcher.SCAN_NAME) || parentContainerName
                                                    .equals(stitcher.STENCIL_NAME))) {
                                        firstFrameFound = true;
                                        IFrame frame = (IFrame) item
                                                .getParentItem();
                                        frame.removeItem(item);

                                        Vector2f itemWorldPos = item
                                                .getWorldLocation();
                                        ...
                                        ((JMERectangularItem) item)
                                                .setSize(
                                                        stitcher.HOTSPOT_FRAME_DIMENSION,
                                                        stitcher.HOTSPOT_FRAME_DIMENSION);
                                        item.centerItem();
                                        clearAllHighlightedHotSpotFrames();
                                        targetFrame
