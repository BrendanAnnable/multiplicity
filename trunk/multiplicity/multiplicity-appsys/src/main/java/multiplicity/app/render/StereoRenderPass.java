package multiplicity.app.render;
...
public class StereoRenderPass extends RenderPass {
    ...
    protected void setFrustum(Camera cam, CameraSide side) {
        ...
        if (projectionMode == ProjectionMode.ASYMMETRIC_FRUSTUM) {
        	...
        	aspectratio = (float)DisplaySystem.getDisplaySystem().getRenderer().getWidth() / (float)DisplaySystem.getDisplaySystem().getRenderer().getHeight();
            aperture = 45.0f;
