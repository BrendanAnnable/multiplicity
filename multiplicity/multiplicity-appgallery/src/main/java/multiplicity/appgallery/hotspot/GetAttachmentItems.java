package multiplicity.appgallery.hotspot;
...
public class GetAttachmentItems extends Thread {
	...
	private HotSpotApp hotspot;
	private String parentContainerName;

	public GetAttachmentItems(HotSpotApp stitcherApp, IPage iPage,
			Vector<IAttachment> attachments, List<IItem> items,
			String parentContainerName) {
		this.hotspot = stitcherApp;
	...
	public File writeFileToLocalStorageDir(String filename, String dirName) {
		String targetDirectory = LocalStorageUtility.getLocalWorkingDirectory(
				hotspot.MULTIPLICITY_SPACE, "").getAbsolutePath()
		...
		if (canUpload) {
			hotspot.output_document_path = targetDirectory
					+ File.separatorChar + filename;
			savedFile = new File(hotspot.output_document_path);
		}
		return savedFile;
	}
			...
			if (iAttachment.isOfImageType()) {
				...
				if (file.exists() == false) {
					WikiUtility wikiUtility = new WikiUtility(
							hotspot.wikiUser, hotspot.wikiPass,
							hotspot.maxFileSize, iAttachment.getMimeType(),
				...
				if (iAttachment.getIsValid()) {
					try {
						img = hotspot.getContentFactory().createImage("photo",
								UUID.randomUUID());
    ...
    public void moveItemToNewFrame(IItem item, MultiTouchCursorEvent event) {
        boolean offParent = true;

        Node s = (Node) hotspot.getOrthoNode();
        ...
        if (parentContainerName.equals(hotspot.BACKGROUND_NAME) && offParent) {
        	IFrame frame = (IFrame) item.getParentItem();
        	frame.removeItem(item);
        	hotspot.moveItemToNewFrame(item, new Vector2f(0.0f, 0.0f), "back-" + item.getUUID());
        } else {
