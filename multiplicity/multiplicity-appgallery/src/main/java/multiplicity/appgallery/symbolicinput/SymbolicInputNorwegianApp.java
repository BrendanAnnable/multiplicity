...
public class SymbolicInputNorwegianApp extends AbstractMultiplicityApp {

	public SymbolicInputNorwegianApp(AbstractSurfaceSystem ass, IMultiTouchEventProducer producer) {
		super(ass, producer);
	}
	...
	public void onAppStart() {
		...
		KeyboardDefinition kbd = new NorwegianKeyboardDefinition();
		...
		kbb.addListener(new IMultiTouchKeyboardListener() {
			...
			public void keyReleased(KeyboardKey k, boolean shiftDown, boolean altDown, boolean ctlDown) {
			    if(k.getKeyCode() == 222) {
                    if(!shiftDown)
                        label2.appendString("æ");
                    else
                        label2.appendString("Æ");
                } else if(k.getKeyCode() == 59) {
                    if(!shiftDown)
                        label2.appendString("ø");
                    else
                        label2.appendString("Ø");
                } else if(k.getKeyCode() == 91) {
	                 if(!shiftDown)
	                     label2.appendString("å");
	                 else
	                     label2.appendString("Å");
	...
	public static void main(String[] args) throws SecurityException, IllegalArgumentException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
		SingleAppMultiplicitySurfaceSystem.startSystem(SymbolicInputNorwegianApp.class);
	}
